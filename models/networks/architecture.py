...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, Block_Name=None, use_rgb=True):
        super().__init__()
        ...
        self.ace_0 = ACE(our_norm_type, fin, 3, ACE_Name=Block_Name + '_ACE_0', status=self.status, spade_params=[spade_config_str, fin, 19], use_rgb=use_rgb)
        ###########  Modifications 1


        ###########  Modifications 1
        self.ace_1 = ACE(our_norm_type, fmiddle, 3, ACE_Name= Block_Name + '_ACE_1', status=self.status, spade_params=[spade_config_str, fmiddle, 19], use_rgb=use_rgb)
        ###########  Modifications 1

        if self.learned_shortcut:
            self.ace_s = ACE(our_norm_type, fin, 3, ACE_Name= Block_Name + '_ACE_s', status=self.status, spade_params=[spade_config_str, fin, 19], use_rgb=use_rgb)
